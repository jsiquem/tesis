...
Se evaluaron cuatro aspectos. El acceso al módulo fue aprobado. La consulta histórica no mostró un cambio observable; la carga no pudo completarse por falta de un archivo de prueba; y las validaciones previas se visualizaron en la pantalla, pero no pudieron comprobarse con un archivo real.
...
\bottomrule
\endlastfoot
FEL-01 & SIS & Acceso y disponibilidad & Iniciar sesión y navegar a Facturas FEL. & El módulo carga y permite consultar sus controles. & La vista cargó y mostró ``POSTGRESQL CONECTADO'', el área de carga y la validación previa. & Fig.~\ref{fig:cap4_fel_pantalla} & Aprobado\\
\addlinespace
FEL-02 & SIS & Consulta histórica & Presionar ``Mostrar datos cargados''. & Debe aparecer el catálogo histórico. & El control permaneció visible, pero no apareció un catálogo adicional. & Fig.~\ref{fig:cap4_fel_pantalla} & No verificable\\
\addlinespace
FEL-03 & SIS & Carga de archivo & Seleccionar un archivo CSV, XLS o XLSX de hasta 15 MB. & El archivo debe pasar a procesamiento. & No se completó porque no había un archivo FEL de prueba disponible. & Fig.~\ref{fig:cap4_fel_pantalla} & No ejecutado\\
\addlinespace
FEL-04 & SIS & Validaciones previas & Revisar el bloque de validación antes del procesamiento. & Deben validarse receptor, NIT, autorización, IVA y duplicados. & Las validaciones se muestran en la interfaz, pero no se comprobaron con un archivo real. & Fig.~\ref{fig:cap4_fel_pantalla} & No verificable\\
\end{xltabular}}
...
\bottomrule
\endlastfoot
Efectividad observable & Casos aprobados / casos evaluados $\times 100$. & 4 & 1 & 25\%\\
\end{xltabular}
...
\addplot[fill=blue!65] coordinates {(FEL,25)};
\end{axis}
\end{tikzpicture}
\caption{Resultado observado de pruebas del módulo de Facturas FEL/SAT.}\label{fig:cap4_grafica_fel_actualizada}
\end{figure}

\noindent\textbf{Interpretación:} El único aspecto aprobado fue la disponibilidad y carga inicial del módulo. La consulta histórica y las validaciones no pudieron confirmarse mediante un cambio visible o un archivo de prueba; por ello, el 25\% representa aprobación observable y no una evaluación completa de todas las funciones FEL.

\subsubsection{Módulo de Proveedores}

Se evaluaron tres aspectos. El directorio y sus datos visibles fueron aprobados, al igual que el cambio de modalidad a partida transitoria. La generación de partidas recibió la interacción, pero no mostró una nueva partida, confirmación o cambio numérico verificable.
...
\bottomrule
\endlastfoot
PROV-01 & SIS & Consulta del directorio & Abrir el módulo y revisar proveedores, estados, crédito, retenciones y cuentas. & El directorio debe mostrar los registros disponibles. & Se visualizaron cuatro proveedores activos con sus condiciones y cuentas contables. & Fig.~\ref{fig:cap4_proveedores_pantalla} & Aprobado\\
\addlinespace
PROV-02 & SIS & Modalidad de partida & Cambiar ``Tipo de partida'' a ``Transitoria general''. & Debe cambiar la modalidad de generación. & La interfaz cambió la modalidad y ocultó el selector individual de proveedor. & Fig.~\ref{fig:cap4_proveedores_pantalla} & Aprobado\\
\addlinespace
PROV-03 & SIS & Generación de partida & Presionar ``Generar partidas'' en modalidad transitoria. & Debe aparecer una confirmación o una nueva partida. & El botón recibió la interacción, pero no se observó un resultado nuevo verificable. & Fig.~\ref{fig:cap4_proveedores_pantalla} & No verificable\\
\end{xltabular}}
...
\bottomrule
\endlastfoot
Efectividad observable & Casos aprobados / casos evaluados $\times 100$. & 3 & 2 & 66.7\%\\
\end{xltabular}
...
\addplot[fill=green!55!black] coordinates {(Proveedores,66.7)};
\end{axis}
\end{tikzpicture}
\caption{Resultado observado de pruebas del módulo de Proveedores.}\label{fig:cap4_grafica_proveedores_actualizada}
\end{figure}

\noindent\textbf{Interpretación:} Se aprobaron la consulta del directorio y el cambio de modalidad de partida. La generación de una partida transitoria no pudo confirmarse porque la interfaz no mostró una respuesta observable después de presionar el botón.

\subsubsection{Módulo de Nomenclatura Contable}

Se evaluaron tres aspectos. La carga del catálogo y la búsqueda por código fueron aprobadas. La apertura del formulario de nueva cuenta no produjo un formulario visible, por lo que no se confirmó el registro ni la selección de naturaleza contable.
...
\bottomrule
\endlastfoot
NOM-01 & SIS & Carga del catálogo & Abrir Nomenclatura y revisar sus indicadores. & Deben cargarse las cuentas y el estado del catálogo. & Se visualizaron 39 cuentas, 4 tipos registrados y estado ``Activo''. & Fig.~\ref{fig:cap4_code_nomenclatura} & Aprobado\\
\addlinespace
NOM-02 & SIS & Búsqueda por código & Ingresar el código 610112 en el buscador. & Debe mostrarse la cuenta coincidente. & El catálogo se filtró a ``DEPRECIACIONES Y COMBUSTIBLES'', tipo Gasto, naturaleza Debe y estado Activa. & Fig.~\ref{fig:cap4_code_nomenclatura} & Aprobado\\
\addlinespace
NOM-03 & SIS & Nueva cuenta & Presionar ``Nueva cuenta''. & Debe abrirse el formulario de registro. & El botón recibió la interacción, pero no apareció un formulario visible. & Fig.~\ref{fig:cap4_code_nomenclatura} & No verificable\\
\end{xltabular}}
...
\addplot[fill=orange!80!black] coordinates {(Nomenclatura,66.7)};
\end{axis}
\end{tikzpicture}
...
\noindent\textbf{Interpretación:} Se aprobaron la carga del catálogo y la búsqueda de una cuenta por código. La apertura de una nueva cuenta no pudo confirmarse porque no apareció un formulario visible después de la interacción.

\paragraph{Resumen de la ejecución}
En conjunto se evaluaron diez aspectos: cuatro en Facturas FEL/SAT, tres en Proveedores y tres en Nomenclatura contable. Se obtuvieron cinco casos aprobados, cuatro no verificables y uno no ejecutado. La efectividad observable global fue de 5 casos aprobados de 10 evaluados, equivalente al 50\%. Este resultado describe el comportamiento observado durante la ejecución del prototipo y no debe interpretarse como una certificación del sistema productivo.
